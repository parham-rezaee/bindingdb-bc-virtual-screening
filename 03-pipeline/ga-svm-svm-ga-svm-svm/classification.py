...
class_names = ['N_A', 'EGFR+HER2', 'ER-beta', 'NFKB', 'PR']
...
for i, color in zip(range(n_classes), colors):
    plt.figure()
    plt.plot(fpr[i], tpr[i], label='ROC curve (area = %0.2f)' % roc_auc[i], color=color)
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    ...
    plt.savefig('ROC_curve_class_' + class_names[i] + '.png', dpi=300)
