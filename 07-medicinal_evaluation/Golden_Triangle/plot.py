...
for file_name, color, title in zip(file_names, colors,title):
    ...
    plt.scatter(filtered_bindingdb_df['LogD'], filtered_bindingdb_df['MW'], color=color, alpha=0.1)
    plt.scatter(filtered_samples_df['LogD'], filtered_samples_df['MW'], color=color)
    plt.scatter(filtered_virtual_screening_df['LogD'], filtered_virtual_screening_df['MW'], color='tab:red', alpha=0.5)
    square_vertices = [(-2, 200), (1.5, 500), (5, 200)]
